...
import {
  LOAD_CITIES_API,
  LOAD_CITIES_API_SUCCESS,
  LOAD_CITIES_API_ERROR,
} from "./typeCitiesData";
...
export const citiesApiCall = (param, searchValue) => {
  return (dispatch) => {
    ...
    axios
      ...
      .catch((err) => {
        const message =
          err.message === "Request failed with status code 504"
            ? `Impossible d'afficher de la donnée. Limite de résultats atteinte. Merci de préciser votre recherche.`
